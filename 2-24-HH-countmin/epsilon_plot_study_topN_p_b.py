...
threshold_rate_vector =[6400000,18750,120,32000000,50000,140]

for ti in range(0,1):
	threshold_rate=threshold_rate_vector[ti]
	threshold_rate_str = str(threshold_rate)#"0.0005"

	summary_file_name = "summary_average_diff_b_p_f.csv"
	...
	for i in range(dim-1,dim):
		...
		ip_filter_b, ip_filter_freq_b = read_csv_data(file_name, 1, 3)
		ip_filter_freq_b, ip_filter_b = (list(x) for x in zip(*sorted(zip(ip_filter_freq_b, ip_filter_b))))
		
		ip_filter_p, ip_filter_freq_p = read_csv_data(file_name, 5, 7)
		ip_filter_freq_p, ip_filter_p = (list(x) for x in zip(*sorted(zip(ip_filter_freq_p, ip_filter_p))))
		
		ip_filter_f, ip_filter_freq_f = read_csv_data(file_name, 9, 11)
		ip_filter_freq_f, ip_filter_f = (list(x) for x in zip(*sorted(zip(ip_filter_freq_f, ip_filter_f))))
		
		
		ip_table_b, ip_table_freq_b = read_csv_data(file_name, 13, 15)
		ip_table_freq_b, ip_table_b = (list(x) for x in zip(*sorted(zip(ip_table_freq_b, ip_table_b))))
		
		ip_table_p, ip_table_freq_p = read_csv_data(file_name, 17, 19)
		ip_table_freq_p, ip_table_p = (list(x) for x in zip(*sorted(zip(ip_table_freq_p, ip_table_p))))
		
		ip_table_f, ip_table_freq_f = read_csv_data(file_name, 21, 23)
		ip_table_freq_f, ip_table_f = (list(x) for x in zip(*sorted(zip(ip_table_freq_f, ip_table_f))))
		
		
		ip_true_b, ip_true_freq_b = read_csv_data(file_name, 25, 27)
		ip_true_freq_b, ip_true_b = (list(x) for x in zip(*sorted(zip(ip_true_freq_b, ip_true_b))))
		
		ip_true_p, ip_true_freq_p = read_csv_data(file_name, 29, 31)
		ip_true_freq_p, ip_true_p = (list(x) for x in zip(*sorted(zip(ip_true_freq_p, ip_true_p))))
		
		ip_true_f, ip_true_freq_f = read_csv_data(file_name, 33, 35)
		ip_true_freq_f, ip_true_f = (list(x) for x in zip(*sorted(zip(ip_true_freq_f, ip_true_f))))
		
		diff_num_filter_b_p = differ_data(ip_filter_b, ip_filter_p,400)
		diff_num_filter_b_f = differ_data(ip_filter_b, ip_filter_f,400)
		
		diff_num_table_b_p = differ_data(ip_table_b, ip_table_p,400)
		diff_num_table_b_f = differ_data(ip_table_b, ip_table_f,400)
		
		diff_num_true_b_p = differ_data(ip_true_b, ip_true_p,400)
		diff_num_true_b_f = differ_data(ip_true_b, ip_true_f,400)
		
		print diff_num_filter_b_p,  diff_num_filter_b_f, diff_num_table_b_p, diff_num_table_b_f, diff_num_true_b_p, diff_num_true_b_f
		
		addToFile(summary_file_name, str(diff_num_filter_b_p)+",") 
		addToFile(summary_file_name, str(diff_num_filter_b_f)+",")
		addToFile(summary_file_name, str(diff_num_table_b_p)+",")
		addToFile(summary_file_name, str(diff_num_table_b_f)+"\n")
